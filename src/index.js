...
module.exports = function (layoutData, opts) {
  ...
  console.log('layoutData', layoutData); // UI 模块结构组成和样式描述等
    ...
    function generateXML(json, options) {
      ...
      if (!!json.length && typeof json != 'string') {
        ...
        if (dataBindingLineType.styleLevel.length > 0) {
          ...
          styleParts.push(
            _line(`className={styles.${className}} style={{`, {
              indent: { tab: indent + 1 }
            }),
            ...dataBindingLine,
          ...
          stylePartsUseClass = [
            _line(`className={styles.${className}}`, { indent: { tab: indent + 1 } })
          ];
        }
  ...
  let _rImport = [
    ...
    _line("import styles from './style.less'", { indent: { tab: 0 } }), // Import Less
    _line("", { indent: { tab: 0 } })
  ];
